...
export class VentasService {
  ...
  async insertar(body: CreateVentaDTO) {
    ...
    body.detalleVenta.forEach((obj) => {
      const data = obj.toArray();
      data.unshift(result.p_id);
  ...
  async actualizar(body: UpdateVentaDTO) {
    ...
    body.detalleVenta.forEach((obj) => {
      this.db.executeFunctionWrite(
        `fn_actualizar_${this.fnName2}`,
        obj.toArray(),
      );
    });
    return result;
  ...
  async buscarDetallesVenta(ideVent: number) {
    return this.db.executeFunctionRead(`fn_filtrar_${this.fnName2}`, [
      ideVent,
      null,
    ]);
  }
